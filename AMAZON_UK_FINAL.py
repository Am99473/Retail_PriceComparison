...
def Amazon_UK(input1):
        ...
        for i in input1:
            ...
            try:
                if Element_not_Found in html:
                    ...
                    Item_Title = driver.find_element(By.XPATH, '//h2[@class="a-size-medium a-spacing-none a-color-base a-text-normal"]')
                    
                    item_link = driver.find_element(By.XPATH, '//a[@class="a-link-normal s-line-clamp-2 s-link-style a-text-normal"]')
                    ...
                    for ii, iii in zip(Product_category,Product_price):
                        ...
                        l.append(['Amazon UK'] +  [i] + [ii.text] + [re.findall(r'\d+\.\d+', iii.text[1:])[0]] + [item_link])
                        # print(re.findall(r'\d+\.\d+', ii.text))
    ...
    data.to_excel('assets/output/AMAZON_UK {}.xlsx'.format(str(input1[0])))

    return data


# Amazon_UK( [9780552166775, 9781784851866] )
